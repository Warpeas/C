...
bool com(vertex &v1, vertex &v2) {
  ...
  // }
  if (tmp == 0) {
    bool flag = dis(vertexes[0], v1) > dis(vertexes[0], v2);
    if (flag) {
      v2 = v1;
    } else {
      v1 = v2;
...
vector<vertex> graham_scan(int n) {
  ...
  for (int i = 3; i < n; ++i) {
    while (top > 0 && cross_product(v[top-1], vertexes[i], v[top]) >
                          0 //  may inside the new triangle, left of v
                            //  right_most to vertexes[i]
           //        cross_product(vertexes[0], v[top], vertexes[i]) > 0 &&
           //        cross_product(vertexes[0], v[top], v[top - 1]) != 0 &&
           // && cross_product(vertexes[0], vertexes[1],
           //                vertexes[top]) != 0 //  not on the bottom line
    ) {
      --top;
      v.pop_back();
    }
...
int main() {
  ...
  if (n < 4) {
    ...
    if (n > 8) {
      ...
      //     verts.push(v);
      // }
      for (vector<vertex>::iterator v = vertexList.begin();
           v != vertexList.end(); v++) {
        long long x = v->x;
        long long y = v->y;
        if (!(x > minX && x < maxX && y > minY && y < maxY))
